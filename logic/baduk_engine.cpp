...
static uint8_t baduk_on_board(int row, int col) {
    return row >= 0 && row < BADUK_BOARD_SIZE && col >= 0 && col < BADUK_BOARD_SIZE;
}
...
uint8_t baduk_load_game(BadukState* state, const BadukGameRecord* games, uint16_t game_count, uint16_t game_index) {
...
static uint8_t baduk_group_has_liberty(BadukState* state, uint8_t start_row, uint8_t start_col, uint8_t visited[BADUK_BOARD_SIZE][BADUK_BOARD_SIZE]) {
    uint8_t color;
...
static void baduk_remove_group(BadukState* state, uint8_t start_row, uint8_t start_col) {
    uint8_t color;
    uint8_t stack_row[BADUK_BOARD_SIZE * BADUK_BOARD_SIZE];
    uint8_t stack_col[BADUK_BOARD_SIZE * BADUK_BOARD_SIZE];
    uint16_t stack_size;
...
uint8_t baduk_play_move(BadukState* state, uint16_t encoded_move, uint8_t color) {
    uint8_t row;
    uint8_t col;
    uint8_t enemy;
    uint8_t i;
