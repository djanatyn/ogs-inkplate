...
static void display_draw_game_info(Inkplate* display, BadukState* baduk_state, uint16_t total_games);
...
static void display_draw_board(Inkplate* display) {
    ...
    // Draw board edge
    display->drawRect(display_pixel_x(0), display_pixel_y(0), GRID_SPACING * (BADUK_BOARD_SIZE - 1),
...
static void display_draw_grid_lines(Inkplate* display) {
    ...
    for (int col = 0; col < BADUK_BOARD_SIZE; col++) {
        ...
        display->drawLine(x, display_pixel_y(0), x, display_pixel_y(BADUK_BOARD_SIZE - 1), BLACK);
    }

    // Draw horizontal lines
    for (int row = 0; row < BADUK_BOARD_SIZE; row++) {
        int y = display_pixel_y(row);
        display->drawLine(display_pixel_x(0), y, display_pixel_x(BADUK_BOARD_SIZE - 1), y, BLACK);
    }
}
    ...
    for (int row = 0; row < BADUK_BOARD_SIZE; row++) {
        for (int col = 0; col < BADUK_BOARD_SIZE; col++) {
            ...
            if (cell == BADUK_BLACK) {
                ...
                display->fillCircle(display_pixel_x(col), display_pixel_y(row), STONE_RADIUS, BLACK);
            } else if (cell == BADUK_WHITE) {
                // White stone - outlined circle
                display->drawCircle(display_pixel_x(col), display_pixel_y(row), STONE_RADIUS, BLACK);
                // Fill with white by drawing a slightly smaller filled white circle
                display->fillCircle(display_pixel_x(col), display_pixel_y(row), STONE_RADIUS - 1, WHITE);
            }

            // Mark the last stone with a small circle (inverted color for visibility)
            if (row == baduk_state->last_row && col == baduk_state->last_col && baduk_state->move_index > 0) {
                int x = display_pixel_x(col);
                int y = display_pixel_y(row);
                int markerRadius = 3;
...
static void display_draw_game_info(Inkplate* display, BadukState* baduk_state, uint16_t total_games) {
    ...
    memcpy_P(&metadata, &GAMES_METADATA[baduk_state->game_index], sizeof(GameMetadata));
